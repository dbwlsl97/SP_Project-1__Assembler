...
static int assem_pass2(void)
{
	...
	for (int i = 0; i < token_line; i++) {
			...
			if (!strcmp(inst_table[opindex]->format, "3/4")) {
				if (strstr(token_table[i]->operator,"+")) { //4형식일때 처리
					...
					if (strchr(token_table[i]->operand[0], '#')) { //직접
						...
						strcpy(temp, lit_table[i].literal); //먼저 temp 에 리터럴 값을 복사하고
						lit = strtok(temp, "'"); // "'" 을 기준으로 잘라줍니다. ----> ex) =C'EOF' 를 "'"를 통해 2번잘라주면 EOF 가 나옴
		...
		else if (!strcmp(token_table[i]->operator,"WORD")) { // 미완성
...
void make_objectcode_output(char *file_name)
{
	...
	if (file_name == NULL)
		...
		for (int i = 0; token_table[i]->sec_addr == 0; i++) { //처음 섹션 0일 때의 헤더,텍스트,모디피케이션 부분을 구현합니다.
			if (!strcmp(sym_table[i].symbol, "COPY")) { //처음 주소가 0인 심볼을 만났을 때, 
				fprintf(object_output, "H%s\t%06X%06X\n", sym_table[i].symbol, sym_table[i].addr, endaddr[0]); // 미리 저장해놓았던 시작주소와 섹션의 전체 길이 
			}
			if (!strcmp(token_table[i]->operator,"EXTDEF")) { //EXTDEF 이 나오면 
				fprintf(object_output, "D");
				...
				for (int j = 0; j < token_line; j++) {
					if (!strcmp(token_table[i]->operand[2], sym_table[j].symbol))
						fprintf(object_output, "%s%06X\n", token_table[i]->operand[2], sym_table[j].addr);
				}
			}
			else if (!strcmp(token_table[i]->operator,"EXTREF")) { //EXTREF 가 나오면
				fprintf(object_output, "R"); 
			...
			op = search_opcode(token_table[i]->operator); // OPCODE를 찾습니다.
			if (op >= 0) { //명령어 테이블에 있다면 ,
				if (!strcmp(sym_table[i].symbol, "FIRST")) { // 심볼 FIRST 부터 시작하고
					fprintf(object_output, "T%06X", sym_table[i].addr); 
					fprintf(object_output, "1D%"); //미완성
				}
				fprintf(object_output, "%06X", token_table[i]->obcode);
			}
			else {
				if (strchr(token_table[i]->operand[0], '+')) { // 오퍼랜드의 첫 번째 글자가 + 라면
					fprintf(object_output, "M"); //4형식이므로 M에 값을 나타냄
					token_table[i]->addr += 1;
					check = token_table[i]->obcode & 32; //비트 연산으로 0의 개수를 셈
					
					fprintf(object_output, "%06X", token_table[i]->obcode);
				}
				else if (!strcmp(token_table[i]->operand[0], "WORD")) {
					fprintf(object_output, "M"); //WORD일 때도 M (BUFEND - BUFFER 가 있기에 M에 값을 써줌)
					token_table[i]->addr += 1;
					check = token_table[i]->obcode & 32;
					fprintf(object_output, "%06X06", token_table[i]->obcode);
				}
